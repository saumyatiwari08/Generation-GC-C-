...
Iter<T> begin() { 	 	
int siz;
if(isArray) siz = arraySize;
else siz = 1;
return Iter<T>(addr, addr, addr + siz);
}
...
return Iter<T>(addr + siz, addr, addr + siz);
}
...
//cout<<"Objects have been promoted to generation 2\n";
#endif
...
if(flag==1)
{
	//cout<<"Objects are in generation 1\n\n";
	flag=0;
}
